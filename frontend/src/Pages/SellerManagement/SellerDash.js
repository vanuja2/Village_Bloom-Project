...
function SellerDash() {
  ...
  useEffect(() => {
    const sellerID = localStorage.getItem('sellerID'); // Get sellerID from local storage
    if (!sellerID) {
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`http://localhost:8081/product?sellerID=${sellerID}`);
        const filteredProducts = response.data.product.filter(product => product.sellerID === sellerID);
  ...
  const generatePDF = () => {
    ...
    doc.autoTable({
      head: [['Product Name', 'Price', 'Description', 'Quantity', 'Category']],
      body: tableData
    });
        ...
        <div className='continer_sub'>
          <div className='admin_btn_nav_con'>
            <button
              ...
              onClick={() => (window.location.href = '/sellerAddProduct')}>Add Product</button>
                    ...
                    <td>
                      <div className='btn_con_tbl'>
                        <button
                          ...
                          onClick={() => (window.location.href = `/sellerUpdateProduct/${product._id}`)}>Update</button>
                        <button
